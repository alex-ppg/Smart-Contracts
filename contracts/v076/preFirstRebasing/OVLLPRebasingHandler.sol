...
contract OVLLPRebasingHandler is OVLBase, IOVLTransferHandler {
    ...
    function handleTransfer(address sender, address recipient, uint256 amount) external override {
        ...
        uint256 senderNewBalances = senderInfo.maturedBalance.sub(amount);
        uint256 recipientNewBalances = recipientInfo.maturedBalance.add(amount);

        senderInfo.maturedBalance = senderNewBalances;
        senderInfo.maxBalance = senderNewBalances;

        recipientInfo.maturedBalance = recipientNewBalances;
        recipientInfo.maxBalance = recipientNewBalances;
